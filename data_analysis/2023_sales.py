##### This script gives a dictionary of Used products and its total paybles in 2023
        ...
        for i, cell in enumerate(column):
            if i > 0:
              ...
              if date > '2023-01-01' and date < '2023-12-31':
                  product_details_in_2023.update({data_sheet.cell(row=i+1, column=6).value:data_sheet.cell(row=i+1, column=15).value})
print(product_details_in_2023)

def addlabels(product_names,product_values):
    for i in range(len(product_names)):
        plt.text(i,product_values[i],product_values[i])
        
product_names = list(product_details_in_2023.keys())
product_values = list(product_details_in_2023.values())

plt.bar(product_names, product_values)
addlabels(product_names, product_values)
plt.xlabel('Product Name')
plt.ylabel('Total Payble for the Product')
plt.title('Paybles for Products sold in 2023')
plt.show()